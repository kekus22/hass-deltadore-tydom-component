...
class HaClimate(ClimateEntity, HAEntity):
    ...
    def hvac_mode(self) -> HVACMode:
        ...
        if hasattr(self._device, "localMode"):
            hvac_mode = getattr(self._device, "localMode", None)
            if hvac_mode is not None and hvac_mode in self.dict_modes_dd_to_ha:
                LOGGER.debug("local_mode = %s", self.dict_modes_dd_to_ha[hvac_mode])
        ...
        if hasattr(self._device, "localMode"):
            ...
            if hvac_mode in ("HEATING", "NORMAL"):
                if hasattr(self._device, "setpoint"):
                    ...
                    override_setpoint = getattr(self._device, "overrideSetpoint", None)
                    if override_setpoint is not None:
                        return float(override_setpoint)
